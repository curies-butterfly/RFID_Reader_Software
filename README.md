...
### 串口配置命令
（上电10s设置），否则默认原配置；
例如发送：#4G=0;ETH=0;LORA=1;TYPE=XY;WIFI=self_test,147258368;MQTT=mqtt://8.159.138.144;EPCBB_TYPE=N704
...
| 参数  | 值  | 说明            |
| ------| -- | ------------- |
| `TYPE`| XY | 标签类型标识，杭州星沿科技标签 |
| `TYPE`| YH | 标签类型标识，杭州悦和科技标签 |
...
| 参数     | 值                     | 说明                                    |
| ------ | --------------------- | ------------------------------------- |
| `EPCBB_TYPE` | `N704`、`N604` | 为了适配RFID模组 N704和N604 所配置基带速率的指令不同，具体看rfidmodule.c中，若不加，则保持默认配置，一般新模组第一次上电后需要配置该参数 |

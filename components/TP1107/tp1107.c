...
static const struct at_urc tp1107_urc_table[] = {
    {"joined", "\r\n", urc_func1},
...
// static inline void tp1107_on_comm_ok(void)
// {
//     tp1107.net_comm_ok = 1;
//     tp1107.net_err = 0;
//     tp1107.last_err_code = 0;
...
static inline void tp1107_on_comm_ok(void)
{
    ...
    if (tp1107.net_joined)
    {
        // 已入网：保持常亮，停止其他模式
        led_indicator_stop(s_led_net_status_handle, BLINK_CONNECTING);
        led_indicator_stop(s_led_net_status_handle, BLINK_RECONNECTING);
        led_indicator_start(s_led_net_status_handle, BLINK_CONNECTED);
    }
    else
    {
...
static inline void tp1107_on_error(int code)
{
    tp1107.net_err = 1;
    tp1107.net_comm_ok = 0;
    tp1107.last_err_code = code;
    if (!s_led_net_status_handle)
    {
        return;
    }
    ...
    {
        led_indicator_stop(s_led_net_status_handle, BLINK_CONNECTING);
        led_indicator_start(s_led_net_status_handle, BLINK_RECONNECTING);
        return;
    }
    // 未入网：ERROR,3 -> 慢闪；其他错误 -> 快闪
    if (code == 3)
    {
        led_indicator_start(s_led_net_status_handle, BLINK_CONNECTING);
    }
...
void urc_func1(struct at_client *client, const char *data, size_t size)
{
    ...
    tp1107.net_comm_ok = 0;
    tp1107.net_err = 0;
    printf("tp1107 joined gateway!!!!!!!!!!!!");
    // 加：入网成功后强制切到常亮，并停止其他模式
    if (s_led_net_status_handle)
    {
        led_indicator_stop(s_led_net_status_handle, BLINK_RECONNECTING);
        led_indicator_stop(s_led_net_status_handle, BLINK_CONNECTING);
...
void urc_func_error(struct at_client *client, const char *data, size_t size)
{
    ...
    int code = 0;
    const char *p = NULL;
    printf("urc_func_error@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
    if (data && (p = strstr(data, "ERROR,")) != NULL)
    {
        code = atoi(p + 6);
    }
    // tp1107_on_error(code);
    ...
    // }
    // 未入网：ERROR,3 -> 慢闪；其他错误 -> 快闪
    led_indicator_stop(s_led_net_status_handle, BLINK_CONNECTED);//停止常亮
    if (code == 3)
    {
        led_indicator_start(s_led_net_status_handle, BLINK_CONNECTING);
    }
    ...
    {
        led_indicator_start(s_led_net_status_handle, BLINK_RECONNECTING);//快闪
    }
}
...
static int unb_data_send_direct(at_device_tp1107 *ins, uint8_t *data, uint8_t len)
{
    ...
    // snprintf(str, sizeof(str), "AT+UNBSEND=%d,%s,1\r\n", len, (const char *)data);// 组合并发送 AT 命令
    // ESP_LOGI(TAG, "%s", str);
    ...
    //     // return -1;
    //     ESP_LOGI(TAG," [%s] AT client send commands failed, response error or timeout !", ins->device_name);
    //     // tp1107_on_error(4); // 设备忙/发送失败，按需映射错误码
    //     led_indicator_start(s_led_net_status_handle, BLINK_RECONNECTING);// 超时或客户端错误视为设备忙/链路异常 -> 快闪
    //     return -1;
    ...
    // // 解析返回：先查错误，再查成功
    // // char *err_line = at_resp_get_line_by_kw(ins->at_resp, "ERROR,");
    // // if (err_line != NULL) {
    // //     int code = atoi(err_line + 6); // "ERROR," 之后的数字
    // //     tp1107_on_error(code);
    // //     return -1;
    // // }
    //  // 先查 ERROR,<code>
    ...
    // if (err_line != NULL) {
    //     int code = atoi(err_line + 6); // "ERROR," 之后的数字
    //     ESP_LOGW(TAG, "UNBSEND failed: ERROR,%d", code);
    //     if (code == 3) {
    //         // 未入网：维持慢闪
    //         led_indicator_start(s_led_net_status_handle, BLINK_CONNECTING);
    //     } else {
    ...
    // // 再查成功关键字
    // if (NULL != at_resp_get_line_by_kw(ins->at_resp, "SENT")) {
    //     ESP_LOGD(TAG," [%s] send successful", ins->device_name);
    //     // 认为通信正常（如果你只在 +NNMI 时常亮，可以把这行删掉）
    //     led_indicator_start(s_led_net_status_handle, BLINK_CONNECTED);
    //     return 0;
    // }

    // // 未匹配到成功/错误关键字：按通用错误处理为快闪
    // led_indicator_start(s_led_net_status_handle, BLINK_RECONNECTING);
    // return 1;
    ...
    if (len >= 128)
        return 1;
    // snprintf(cmd, sizeof(cmd), "AT+UNBSEND=%d,%s,1\r\n", len, (const char *)data);
    if (at_obj_exec_cmd(at_client_get(UART_NUM_2), ins->at_resp_long, cmd) != RT_EOK)
    {
        led_indicator_start(s_led_net_status_handle, BLINK_RECONNECTING);
        return -1;
    }
    char *err_code_line = at_resp_get_line_by_kw(ins->at_resp_long, "ERROR,");
    if (err_code_line != NULL)
    {
        int code = atoi(err_code_line + 6);
        tp1107_on_error(code);
        return -1;
    }
    char *sent_line = at_resp_get_line_by_kw(ins->at_resp_long, "SENT");
    if (sent_line != NULL)
    {
        if (at_resp_get_line_by_kw(ins->at_resp_long, "OK") != NULL)
        {
            tp1107_on_comm_ok();
            return 0;
        }
        if (at_resp_get_line_by_kw(ins->at_resp_long, "ERROR") != NULL)
        {
            tp1107_on_error(5);
            return -1;
        }
...
static void unb1_entry(void *parameter)
{
    ...
    {
        // 接收消息长度
        if (xQueueReceive(mq_1, &len, portMAX_DELAY) == pdTRUE)
        { // 从消息队列mq_1中接收数据长度len,阻塞等待portMAX_DELAY
            if (len > 0)
            {
                // 接收实际数据
                if (xQueueReceive(mq_1, unbbuffer1, portMAX_DELAY) != pdTRUE)
                { // 再从同一个队列mq_1中读取实际数据内容，存储到unbbuffer1
...
RESTART:
    ...
    if (1 == once)
    {
        ...
        unb_device->at_resp = at_create_resp(256, 0, pdMS_TO_TICKS(4000));
        unb_device->at_resp_long = at_create_resp(256, 0, pdMS_TO_TICKS(75000));
    }
    ...
    while (!unb_device->net_joined)
    {
        ...
        if (at_obj_exec_cmd(at_client_get(UART_NUM_2), unb_device->at_resp, "AT+JOIN?\r\n") == RT_EOK)
        {
            if (at_resp_get_line_by_kw(unb_device->at_resp, "joined") != NULL)
            {
